...
export 'src/batch_service.dart';
export 'src/escrow_service.dart';
export 'src/payment_service.dart';
export 'src/escrow_preflight.dart';
export 'src/escrow_preflight_client.dart';
export 'src/batch_utils.dart';
...
class XRPLutter {
  ...
  // セッション/接続
  Future<WalletSession> connectWallet(
      {required WalletProvider provider}) async {
  ...
  Future<TransferResult> transferNft({
    ...
    if (metadataJson != null && MetadataUtils.isSoftSbtJson(metadataJson)) {
      if (blockIfSoftSbt) {
        ...
        print(
            '[XRPLutter] Warning: Soft SBTフラグが付与されたNFTの送付です。アプリ外では転送可能であるためご注意ください。');
      }
    }
    final account = await _wallet.getAccountInfo();
  ...
  Future<BurnResult> burnNft({required String nftId}) async {
    final account = await _wallet.getAccountInfo();
    final txJson =
        _nft.buildBurnTxJson(accountAddress: account.address, nftId: nftId);
    final submit = await _wallet.signAndSubmit(txJson: txJson);
    final hash = submit['result']?['hash'] ?? 'dummyHash';
  ...
  Future<Map<String, dynamic>> awaitTransaction(String hash,
      {Duration timeout = const Duration(seconds: 20),
      Duration pollInterval = const Duration(milliseconds: 800)}) {
    return _client.awaitTransaction(hash,
